...
Для существенной части количественных переменных в данном наборе характерны "хвосты" в правой части распределения. Чтобы приблизить эти распределения к нормальным было применено логарифмирование. Кроме того, часть перменных попарно скореллированы (например, `Administrative` и `Administrative Duration`), что позволяет исключить одну переменную из каждой пары. Это позволило улучшить качество моделей.
...
2. Предиктор имеет близкую к нулю или отрицательную Permutation importance
...
Для дополнительной наглядности анализа предложенных моделей был построе Exlainer Dashboard. В частности, он был построен для последней версии KNN классификатора на 100 объектах и предсказаниях модели. С помощью техники SHAP была исследована важность предикторов. Оказалось, что наиболее важными являются следующие колонки:

1. PageValues_log (с большим отрывом)
2. Month_Nov
3. Month_May
4. Informational_0
5. BounceRates_log

Также с помощью Explainer Dashboard были изучены основные метрики качества модели.
